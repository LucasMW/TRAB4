...
static const char OBTER_VERTICES_CMD	[]="=obtervertices";
static const char OBTER_ARESTAS_CMD	[]="=obterarestas";
#define TRUE  1
#define FALSE 0
...
*	  =obtervertices		        inxGrafo  CondRetEsp
*	  =obterarestas					inxGrafo  noId CondRetEsp
************************************************************************/
   ...
   TST_tpCondRet TST_EfetuarComando( char * ComandoTeste )
   {
      ...
      /* Testar excluir aresta */
		  ...
		  else if (strcmp( ComandoTeste , IR_VIZINHO_CMD ) == 0)
		 {
		  ...
		  else if (strcmp( ComandoTeste ,  OBTER_VERTICES_CMD) == 0)
		 {
			 numLidos = LER_LerParametros( "ii",&inxGrafo, &CondRetEsp) ;
			  if ( ( numLidos != 2 )|| ( ! ValidarInxGrafo( inxGrafo , NAO_VAZIO )) )
            {
			  ...
			  CondRet =(TST_tpCondRet) GRA_ObterVertices(VTGRAFO[inxGrafo],&intpointer,&i);
			 if(CondRet==GRA_CondRetOK)
			{	printf("\n\nObterVertices: < ");
				for(j=0;j<i;j++)
					printf("%d, ",intpointer[j]);
				printf(">\n");
			 }
			    return TST_CompararInt( CondRetEsp, CondRet, "Condição de retorno errada na obter vértices");
		 }
		  else if (strcmp( ComandoTeste ,  OBTER_ARESTAS_CMD) == 0)
		 {
			  ...
			  if ( ( numLidos != 3 )|| ( ! ValidarInxGrafo( inxGrafo , NAO_VAZIO )) )
            {
			  ...
			  CondRet =(TST_tpCondRet) GRA_ObterArestasNo( VTGRAFO[ inxGrafo ],noIni,&intpointer,&i) ;
			 if(CondRet==GRA_CondRetOK)
			 {	
				 printf("\n\nArestas do vertice %d: < ",noIni);
				for(j=0;j<i;j++)
					printf("%d, ",intpointer[j]);
				printf(">\n");
			 }
			    return TST_CompararInt( CondRetEsp, CondRet, "Condição de retorno errada na ObterArestas");
		 }
...
/********** Fim do módulo de implementação: TGRA Teste grafo  **********/
